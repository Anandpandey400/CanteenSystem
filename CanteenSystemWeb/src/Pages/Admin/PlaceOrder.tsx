...
export default function PlaceOrder() {
  ...
  useEffect(() => {
    fetchUserWalletDetails(selectedUserId);
  }, [selectedUserId]);
  ...
  const handleConfirmPayment = async () => {
    ...
    try {
      ...
      // 🔴 Business failure (inventory, wallet, cap, etc.)
      if (!data.success) {
      ...
      const message =
        error?.response?.data?.message || "Insufficient inventory";
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
              {filteredMenuItems.map((item) => {
               ...
               whitespace-nowrap overflow-hidden text-ellipsis"
                        title={item.menuName} // 👈 show full text on hover
                      >
                        {item.menuName}
                      </h3>
